...
Report bugs at https://github.com/roocs/clisops-core/issues.
...
clisops-core could always use more documentation, whether as part of the official clisops-core docs, in docstrings, or even on the web in blog posts, articles, and such.

Submit Feedback
~~~~~~~~~~~~~~~

The best way to send feedback is to file an issue at https://github.com/roocs/clisops-core/issues.
...
Ready to contribute? Here's how to set up ``clisops-core`` for local development.

#. First, clone the ``clisops-core`` repo locally.

    * If you are not a ``clisops-core`` collaborator, first fork the ``clisops-core`` repo on GitHub, then clone your fork locally.

        .. code-block:: console

            git clone git@github.com:your_name_here/clisops-core.git

    * If you are a ``clisops-core`` collaborator, clone the ``clisops-core`` repo directly.
...
#. Install your local copy into a development environment. You can create a new Anaconda development environment with:
    ...
    This installs ``clisops_core`` in an "editable" state, meaning that changes to the code are immediately seen by the environment. To ensure a consistent coding style, `make dev` also installs the ``pre-commit`` hooks to your local clone.
...
#. Before merging, your Pull Request will need to be based on the `main` branch of the ``clisops-core`` repository. If your branch is not up-to-date with the `main` branch, you can perform similar steps as above to update your branch:
...
#. Once your Pull Request has been accepted and merged to the `main` branch, several automated workflows will be triggered:
    ...
    - `ReadTheDocs` will automatically build the documentation and publish it to the `latest` branch of `clisops_core` documentation website.
    - If your branch is not a fork (i.e. you are a maintainer), your branch will be automatically deleted.

You will have contributed to ``clisops_core``!
...
#. The pull request should include tests and should aim to provide `code coverage <https://en.wikipedia.org/wiki/Code_coverage>`_ for all new lines of code. You can use the `--cov-report html --cov clisops_core` flags during the call to ``pytest`` to generate an HTML report and analyse the current test coverage.
...
.. code-block:: console
    ...
    python -m pytest tests/test_clisops_core.py

You can also directly call a specific test class or test function using:

.. code-block:: console

    python -m pytest tests/test_clisops_core.py::TestClassName::test_function_name
...
.. code-block:: console
    ...
    python -m ruff check src/clisops_core tests
    python -m flake8 src/clisops_core tests
    validate-docstrings src/clisops_core/**.py
...
Please note that this project is released with a `Contributor Code of Conduct <https://github.com/roocs/clisops-core/blob/main/CODE_OF_CONDUCT.md>`_.
