...
class Grid:
    ...
    def __init__(
        ...
        if isinstance(ds, (xr.Dataset, xr.DataArray)):
            ...
            raise ValueError("xarray.Dataset, grid_id or grid_instructor have to be specified as input.")
    ...
    def _grid_from_id(self, grid_id):
        ...
        try:
            ...
            raise KeyError(f"The grid_id '{grid_id}' you specified does not exist.") from err
    ...
    def _grid_from_instructor(self, grid_instructor: tuple | float | int):
        ...
        if len(grid_instructor) not in [1, 2, 3, 6]:
            raise ValueError("The grid_instructor has to be a tuple of length 1, 2, 3 or 6.")
            ...
            raise ValueError("The grid_instructor variable is malformed.")
    ...
    def grid_reformat(self, grid_format: str, keep_attrs: bool = False) -> None:
        """
        ...
        reformat_operation = f"reformat_{self.format}_to_{grid_format}"
    ...
    def _apply_lsm(self, mask: str | None = None) -> bool:
        ...
        if any(maskvar in self.ds for maskvar in ["sftlf", "lsm"]) and mask is not None:
            ...
            if mask:
                warnings.warn("No land sea mask found in the dataset that could be applied.", stacklevel=2)
        return False
    ...
    def detect_type(self) -> str:
        ...
        # Other formats
            ...
            raise ValueError("Grid type can only be determined for datasets following the CF conventions.")

    def detect_extent(self) -> tuple[str, str]:
        """
        ...
        if xlast > 360 or xfirst < -180 or abs(xlast - xfirst) > 360 or (xlast > 180 and xfirst < 0):
            warnings.warn("Converting grid longitude values to [-180, 180] longitude frame.", stacklevel=2)
        ...
        if self.ds[self.lon].ndim == 2 and self.ds[self.lat].ndim == 2:
            ...
            raise ValueError("Only 1D and 2D longitude and latitude coordinate variables supported.")
    ...
    def _transfer_coords(self, source_grid: Grid, keep_attrs: str | bool = True) -> None:
        ...
        if not keep_attrs:
            ...
            raise ValueError("Illegal value for the parameter 'keep_attrs'.")
...
class Weights:
    ...
    local_weights_dir: str = Path().cwd().as_posix()
    ...
    def __init__(
        ...
        if not isinstance(grid_in, Grid) or not isinstance(grid_out, Grid):
            raise ValueError("Input and output grids have to be instances of clisops.core.Grid.")
    ...
    def _read_info_from_cache(self, key: str) -> str | None:
        ...
        if Path(weights_dir, self.filename).is_file() and Path(weights_dir).joinpath(f"{Path(self.filename).stem}.json").is_file():
            with Path(weights_dir).joinpath(f"{Path(self.filename).stem}.json").open("r") as f:
                weights_dic = json.load(f)
                try:
                    return weights_dic[key]
                except KeyError:
                    warnings.warn(f"Requested info {key} does not exist in the metadata of the cached weights.", stacklevel=2)
                    return
        else:
            return
