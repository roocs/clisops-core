...
clean-docs: ## remove docs artifacts
	rm -f docs/apidoc/clisops_core*.rst
...
lint: install-lint ## check style
	...
	python -m flake8 --config=.flake8 src/clisops_core tests
	python -m numpydoc lint src/clisops_core/**.py
	python -m vulture src/clisops_core tests
	codespell src/clisops_core tests docs
	python -m deptry src
	python -m yamllint --config-file=.yamllint.yaml src/clisops_core
...
coverage: install-test ## check code coverage quickly with the default Python
	python -m coverage run --source src/clisops_core -m pytest
...
autodoc: install-docs clean-docs ## create sphinx-apidoc files:
	sphinx-apidoc -o docs/apidoc --private --module-first src/clisops_core
